...
MAP <- bayestestR::map_estimate(data_irap_n$N) |>
...
data_figure_1 <- data_irap_n_per_study |>
  filter(journal == "IRAP") |>
  group_by(field, year) |>
  summarize(median_n = janitor::round_half_up(median(N, na.rm = TRUE), 0)) |>
  ungroup() |>
...
# Filter out non-social studies published in psych science
data_table_2_filtered <- data_combined |>
